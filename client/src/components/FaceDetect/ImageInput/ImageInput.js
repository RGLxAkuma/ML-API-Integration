...
const Imageinput = (props) => {
  ...
  const changeWindowToFace = () => { // this function is called when "Submit" button is clicked
      ...
      fetch("http://localhost:5000/img_analysis", { //POST request to server API endpoint corresponding to Face Detection
        method: "POST",
        headers: {
  ...
  const fileUploadHandler = (event) => { //this function is to get the local input image from the client
  ...
  const ChangeWindowHandler = () => { //when clicked on Home button the page re-render and goes back to Home Page
    props.onFaceToHome();
  };

  if (facesNumber !== -1) { //contion to switch the page from ImageInput to ImageShowcase
    return (
