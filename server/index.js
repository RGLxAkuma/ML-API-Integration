...
const API_KEY = "Enter_Your_API_Key"; //API key corresponding to Perspective API
const DISCOVERY_URL =
  "https://commentanalyzer.googleapis.com/$discovery/rest?version=v1alpha1";

app.post("/text_analysis", (req, res) => { //creating the API endpoint for text Recognition
  ...
  if (!statement) {
    ...
    google
      ...
      .then((client) => {
        const analyzeRequest = { // sending the JSON data of the input string from the client to the API endpoint of the publicc API
          comment: {  
            text: `${statement}`,
          },
          requestedAttributes: { // requesting for custom parameters
            TOXICITY: {},
            INSULT: {},
            THREAT: {},
          },
        ...
        client.comments.analyze(
          ...
          (err, response) => {
            if (err) {
              res.send({             // sends specific JSON data when unsupported language is inputed
                error: "error",
              });
              return;
            }
            ...
            const tox =response.data.attributeScores.TOXICITY.summaryScore.value;
            ...
            res.send({ //sending response to client with corresponding values
              TOXICITY: `${tox}`,
              INSULT: `${ins}`,
              THREAT: `${trt}`,
...
app.post("/img_analysis", (req, res) => { //creating the API endpoint for Face Detection
  const { img_bs64 } = req.body;


  if (!img_bs64) { //if the base64 data is empty it will throw an error to the forntend
...
//Imagga API Credentials
  ...
  (async () => {
      try {
          const response = await got.post('https://api.imagga.com/v2/faces/detections', {body: formData, username: apiKey, password: apiSecret}); //POST request to the Imagga API endpoint of Face detection
